...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        try {
            this.modelIdField.setDrawsBackground(false);
        } catch (Throwable ignored) {
        }
        ...
        try {
            this.baseUrlField.setDrawsBackground(false);
        } catch (Throwable ignored) {
        }
        ...
        try {
            this.timeoutField.setDrawsBackground(false);
        } catch (Throwable ignored) {
        }
        ...
        try {
            this.maxRetriesField.setDrawsBackground(false);
        } catch (Throwable ignored) {
        }
        ...
        this.languageButton = ButtonWidget.builder(
                ...
                button -> cycleLanguage()).dimensions(centerX - 110, startY + 150, 100, 18).build();
        ...
        this.loggingButton = ButtonWidget.builder(
                ...
                button -> toggleLogging()).dimensions(centerX + 10, startY + 150, 100, 18).build();
        ...
        this.confirmationButton = ButtonWidget.builder(
                ...
                button -> toggleConfirmation()).dimensions(centerX - 50, startY + 180, 100, 18).build();
        ...
        ButtonWidget saveButton = ButtonWidget.builder(
                ...
                button -> saveConfig()).dimensions(centerX - 70, startY + 220, 60, 18).build();
        this.addDrawableChild(saveButton);

        // Cancel button
        ButtonWidget cancelButton = ButtonWidget.builder(
                Text.translatable("gui.promptcraft.config.cancel"),
                button -> this.close()).dimensions(centerX + 20, startY + 220, 60, 18).build();
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer,
                ...
                centerX - 120, startY + 26, 0xFFFFFFFF, true);
        context.drawText(this.textRenderer,
                Text.translatable("gui.promptcraft.config.model_id"),
                centerX - 120, startY + 54, 0xFFFFFFFF, true);
        context.drawText(this.textRenderer,
                Text.translatable("gui.promptcraft.config.base_url"),
                centerX - 120, startY + 82, 0xFFFFFFFF, true);
        context.drawText(this.textRenderer,
                Text.translatable("gui.promptcraft.config.timeout"),
                centerX - 120, startY + 110, 0xFFFFFFFF, true);
        context.drawText(this.textRenderer,
                Text.literal("Max Retries"),
                centerX - 20, startY + 110, 0xFFFFFFFF, true);
    }

    private void cycleLanguage() {
        currentLanguageIndex = (currentLanguageIndex + 1) % languages.length;
        config.language = languages[currentLanguageIndex];
        this.languageButton.setMessage(Text
                .literal("Language: " + LocalizationHelper.getLanguageDisplayName(languages[currentLanguageIndex])));
    }

    private void toggleLogging() {
        config.enableLogging = !config.enableLogging;
        this.loggingButton
                .setMessage(Text.literal("Logging: " + LocalizationHelper.getBooleanText(config.enableLogging)));
    }

    private void toggleConfirmation() {
        config.requireConfirmation = !config.requireConfirmation;
        this.confirmationButton
                .setMessage(Text.literal("Confirm: " + LocalizationHelper.getBooleanText(config.requireConfirmation)));
    }

...
public class BlacklistScreen extends Screen {
    ...
    protected void init() {
        ...
        try { this.addKeywordField.setDrawsBackground(false); } catch (Throwable ignored) {}
        // Placeholder API may not exist on this MC version; fallback by drawing hint text
        ...
        ButtonWidget addButton = ButtonWidget.builder(
            ...
            button -> addKeyword()
        ).dimensions(centerX + 110, startY + 40, 60, 20).build();
        this.addDrawableChild(addButton);
        
        // Remove button
        ButtonWidget removeButton = ButtonWidget.builder(
            Text.translatable("gui.promptcraft.blacklist.remove"),
            button -> removeSelectedKeyword()
        ).dimensions(centerX - 150, startY + 320, 80, 20).build();
        ...
        ButtonWidget scrollUpButton = ButtonWidget.builder(
            Text.literal("↑"),
            button -> scrollUp()
        ).dimensions(centerX + 160, startY + 80, 20, 20).build();
        ...
        ButtonWidget scrollDownButton = ButtonWidget.builder(
            Text.literal("↓"),
            button -> scrollDown()
        ).dimensions(centerX + 160, startY + 280, 20, 20).build();
        ...
        ButtonWidget saveButton = ButtonWidget.builder(
            ...
            button -> saveBlacklist()
        ).dimensions(centerX - 50, startY + 350, 80, 20).build();
        ...
        ButtonWidget cancelButton = ButtonWidget.builder(
            ...
            button -> this.close()
        ).dimensions(centerX + 40, startY + 350, 80, 20).build();
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        renderBlacklistItems(context, centerX - 170, startY + 90, mouseX, mouseY);
        ...
        if (blacklistedKeywords.size() > ITEMS_PER_PAGE) {
            ...
            context.drawTextWithShadow(this.textRenderer, Text.literal(pageInfo),
                centerX + 120, startY + 305, 0xFFFFFF);
        }
    }
    ...
    private void renderBlacklistItems(DrawContext context, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < maxItems; i++) {
            ...
            if (index >= blacklistedKeywords.size()) break;
            ...
            if (index == selectedIndex) {
                context.fill(x - 2, itemY - 1, x + 302, itemY + itemHeight - 1, 0xFF404040);
            } else if (isHovered) {
                context.fill(x - 2, itemY - 1, x + 302, itemY + itemHeight - 1, 0xFF202020);
            }
            
            // Draw keyword
            context.drawTextWithShadow(this.textRenderer, Text.literal(keyword), 
                x, itemY + 4, 0xFFFFFF);
        }
    }
